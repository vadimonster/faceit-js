...
module.exports = class Faceit {
  ...
  championships(championship_id, option, champ_type, limit) {
    if (!limit) limit = 20;
    if (!champ_type) champ_type = 'all';    
    ...
    if (option == "organizer") options.url = `${options.url}?expanded=organizer`;

    options.url = `${options.url}?type=${champ_type}`;
  ...
  organizers(organizers_id, option, limit, type) {
    let options = {
  ...
  players(player_id, option, game) {
    if (!player_id) return this._missing("player_id");
    let options = {
      url: `${this.apiUrl}/players/${player_id}`
    };
    if (option == "history") options.url = `${options.url}/history`;
    if (option == "hubs") options.url = `${options.url}/hubs`;
